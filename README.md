...
# Sandboxed code-eval gate

Untrusted input is a snippet; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a snippet
- **Closed type:** `type Decision = RunSand(SandOp) | RejectEval` where `SandOp` is closed
- **Sink / capability:** `grant runSandbox` → `using runSandbox { privileged { runSandbox(d) } }`
- **Consequence axes:** Trust, Capabilities

## Run the demo

```sh
examples/apps/sandbox-eval/demo.sh
```
...
- `sandbox-eval_safe.eide` — the correct design.
- `sandbox-eval_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
